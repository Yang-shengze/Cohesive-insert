...
# 更加不易出错（对于inp文件格式的依赖降低） 算法优化，如原来10000网格插入需60分钟，现在只需20分钟
# 作者：杨晟泽    时间：2020.4.24
...
file_name = 'lamellar_mesh_test1.inp'          # 要插入的文件名（生成的文件为result.inp） 重要！
text = []                       # 读取文件的信息列表
...
def modify_data():
    ...
    print('nodeappearance',node_appearance)
    for i in element_dict:
        for j in range(3):
...
modify_data()
endtime = datetime.datetime.now()
print('element_dict_modify',element_dict)
...
starttime = datetime.datetime.now()
print('all_dict',cohesive_dict)
print('inner',inter_dict)
print('out',edge_dict)
# 书写数据
